...
$SqlQuery = "SELECT
                CASE 
                WHEN UserStaff.iBaseStaffIDid > 0
                    THEN
                        CONCAT('STAFF-',UserStaff.iBaseStaffIDid)
                    ELSE
                        CONCAT('STAFF-',Staff.iStaffID)
                END as StaffGUID,
                Staff.iSchoolID as SchoolID
            FROM 
                Staff
                LEFT OUTER JOIN UserStaff ON Staff.iStaffID=UserStaff.iStaffID
            WHERE 
                Staff.lInactive=0
                AND Staff.iSchoolID NOT IN (5850953,5850963,5850964,5851066) 
                AND Staff.cLastName NOT LIKE 'ADMIN%' 
                AND Staff.cLastName NOT LIKE '%SubSec%' 
                AND Staff.cLastName NOT LIKE '%egov%' 
